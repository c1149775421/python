...
def extract():
  ...
  zfile = zipfile.ZipFile('office.zip','r')
  for password in MyIter(1,3):#随机迭代出1~4位数的密码，在不明确位数的时候做相应的调整
    if zfile:
      try:
        zfile.extractall(path='.',pwd=str("azrj"+password).encode('utf-8'))
        print ("当前压缩密码为：",("azrj"+password))
        end_time = time.time()
        print ('当前破解压缩包花了%s秒'%(end_time-start_time))
        sys.exit(0)
      except Exception as e:
        print ('pass密码：',("azrj"+password))
        pass
