...
def main(entry):
    ...
    if ent=="当前时间" or ent=="现在时间" or ent=="时间" or ent=="现在几点了":
        print(da, "当前北京时间", time.strftime("%H:%M:%S"))
        flag = True

    if ent=="今天几号" or ent=="当前日期" or ent=="日期":
        print(da,"今天是",time.strftime("%Y-%m-%d"))
        flag = True
    ...
    if ent=="pause" or ent=="stop":
        ...
        print("输入'unpause'或者'pass'可以继续播放哦!")
        flag = True
    if ent=="unpause" or ent=="pass":
